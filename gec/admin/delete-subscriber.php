...
// Query
$sqlStatement = "update `subscribers` set `deleted` = 1, `modified_date` = '" .  date('c') . "' where `subscriber_email` = ";
$result = false;
$failSub = array();
...
if (count($failSub) == 0)
{
	...
	$content .= (count($subscribers) == 1) ? ' subscriber has been deleted.' : ' subscribers have been deleted.';
	$message = array(
		'content' => $content,
		'type' => 'success'
	);
...
else
{
	...
	$content .= (count($subscribers) - count($failSub) == 1) ? ' subscriber has been deleted.' : ' subscribers have been deleted.';
	$message = array(
		'content' => $content,
		'type' => 'success'
	);
	$content = count($failSub);
	$content .= (count($failSub) == 1) ? ' subscriber has failed to be deleted.' : ' subscribers have failed to be deleted.';
	$message_err = array(
		'content' => $content,
		'type' => 'error'
	);
